$(document).ready(function() {
	///////////////////////////compatibility//////////////////////////
	 var isChrome = /Chrome/.test(navigator.userAgent) && /Google Inc/.test(navigator.vendor);
              ...
              // logout
              $('.tooltip').tipsy();
             $('div.do-btngroup a.caret,div.do-btngroup a.rating').click(function(event) {
             	$(this).parent().children('ul').slideDown(200);
             	 event.stopPropagation();
             });
             $(document).click(function(event) {
             	 $('.do-vocab.addtovocab').hide();
             	$('div.logicon div.do-btngroup ul.do-dropdown').slideUp(200);
             });
             // end of log out
             $('.chart').wl_Chart();
	$('.treetable2').hide();
  ...
  //     });
	...
	var $content = $('#content');
		...
		/*----------------------------------------------------------------------*/
		// select,
		$(" input[type=file]").not('select[multiple]').uniform();
	...
	data:[[0,82],[1,27],[2,25],[3,65],[4,79],[5,47]]
		...
		// 	eventSources: [
		// 			{
		// 				url: 'https://www.google.com/calendar/feeds/zh_cn.china%23holiday%40group.v.calendar.google.com/public/basic'
		// 			},{
		...
		$searchfield
			.bind({
				'focus.wl': function(){
		   			...
		   			// $searchfield.select().parent().css("padding-right","60px");
		   			$('button#searchsubmit').show();
				},
				'blur.wl': function(){
	   				 $searchfield.parent().animate({width: '90px'},100);
	   				// $searchfield.select().parent().css("padding-right","0px");
	   				$('button#searchsubmit').hide();
