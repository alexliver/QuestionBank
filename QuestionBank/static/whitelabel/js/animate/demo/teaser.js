$(window).load(function() {	
	...
	function buildMainTimeline() {
		mainTimeline = new TimelineMax({onUpdate:updateSlider, paused:true, onComplete:showFunBox});
	...
	reverse_btn.click(function(){
		if(mainTimeline.progress() == 0){
			mainTimeline.reverse(0);
		}else{
	...
	$("#timelineDemo").click(function(){
		showFunBox();
	});
	
	function showFunBox(){
		TweenLite.to(funBox, 0.5, {css:{top:330}})
	}
	...
	function init() {
		checkVersion();	
		buildMainTimeline();
		TweenLite.to($("#loading"), 1,  {css:{autoAlpha:0}, onComplete:startUp, delay:0.5, immediateRender:true});
		TweenLite.set(headerText, {webkitBackfaceVisibility:"hidden"});
	}
